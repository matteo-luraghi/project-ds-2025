...
/**
 * Implementation of a Vector Clock that represent the temporal advancement of each server.
 * DIMENSION represent the number of servers
 */
public class TimeVector implements Serializable {
  ...
  public TimeVector(int dimension) throws InvalidDimensionException {
    if (dimension < 2) throw new InvalidDimensionException();
   ...
   */
  public TimeVector(int dimension, int[] initValues)
      throws InvalidDimensionException, InvalidInitValuesException {
    if (dimension < 2) throw new InvalidDimensionException();
    if (initValues.length != dimension) throw new InvalidInitValuesException();
  ...
  public void increment(int serverID) throws ArrayIndexOutOfBoundsException {
    if (serverID >= this.dimension || serverID < 0) throw new ArrayIndexOutOfBoundsException();

    this.vector[serverID]++;
  }

  /**
   * returns true iif THIS is smaller than or equal to OTHER in all timestamps, except for the
   * timestamp of the senderID for which THIS can be bigger than OTHER by at most one
   *
   * @throws ImpossibleComparisonException
   */
  public boolean happensBefore(TimeVector other, int senderID)
      throws ImpossibleComparisonException {
    if (other.dimension != this.dimension) throw new ImpossibleComparisonException();

    for (int i = 0; i < this.dimension; i++) {
      int myVector = this.vector[i] - (i == senderID ? 1 : 0);
      if (myVector > other.vector[i]) return false;
    }
  ...
  /**
   * returns the temporal difference THIS - OTHER maybe useless
   *
   * @throws ImpossibleComparisonException
   */
  ...
  /**
   * Allign the timestamps of THIS vector with OTHER vector. for each timestamps set to the max
   * between THIS and OTHER, except for the owner serverID
   *
   * @throws ImpossibleComparisonException
   */
  ...
  public boolean equals(Object other) {
    ...
    if (this.dimension != obj.dimension) return false;
